...
The lambda expression passed to `filter` in the new line uses `getLines`
...
required one new line of code and no changes in exiting code.
...
The pipeline starts with a stream of file names returned by the method `getJavaFiles`
defined above.
...
The method `getLines` is used twice in the pipeline,
...
We use `getLines` in arguments of the `filter` and `flatMap` combinators.
...
The method `getLines` is defined as follows.[^preliminary]
...
As a consequence, we can use `getLines` in predicates and functions
...
could potentially allow language designers to find a more seamless integration.
...
Interestingly, the second use of `getLine` as argument of `flatMap` does not have this problem.
The API documentation for `flatMap` contains the following sentence.

> Each mapped stream is closed after its contents have been placed into this stream.

As a consequence, files are closed correctly when opened for printing matching lines
by the second use of `getLines`.
...
will lead a new stream that contains the same elements as the original stream.
